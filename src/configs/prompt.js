...
3. **剧情深化:** 优先扩展当前事件与已有角色关系，避免轻易引入新人物，出现了角色必须有姓名，且是符合时间背景的三国历史知名人物。
4. **系统整合:** 玩家的**战法/计策**（${gameState.skills.map(s => s.name).join('、') || '无'}）与行囊须自然嵌入剧情与选项中。
    * 战法/计策: 如果**玩家行动**没有特定指明，则根据玩家行动自动选择最合适的战法/计策。
    * 行囊消耗: 如果**玩家行动**没有特定指明，则根据玩家行动自动使用合适的行囊，相应的，使用的行囊需要更新数量，如果数量为0，则从行囊中移除。
    * 行囊更新: 如果玩家获得了新的物品，则需要在输出中更新行囊。
    * 属性体系更新：根据玩家行动和事件结果，自动更新玩家的属性，武力、智力、统率这三项能力基数越大，其增长的幅度会减缓。
5. **剧情推动:** 每段文本结尾必须推动事情发展，严禁使用引导语。
6. **生命值规则:** 非战斗、非负伤、非生病情节不得变更体力 (health)。
7. **循环打破机制 (LOOP BREAKER):**
    * 分析最近5条大事记。
    * 识别重复循环（如缠斗同敌、围城不克）。
    * 强制推进事件（敌败、援军、内讧、奇谋突袭等）。
    * 提供新阶段行为（如“清点战果”、“休整军队”、“转战他地”）。
...
- 60+ 岁（老年）→ 谋后事、终结局
### 核心系统
A) 军事线：应征入伍/募兵起事/依附军阀。需经营资源，凭战功晋升
B) 内政线：担任幕僚或官吏，治理辖区，推行屯田。需平衡民心赋税与人才
C) 外交谋略线：游说诸侯、缔结盟约、联姻离间。需信誉与情报支持
D) 世家大族：宗族联姻、举荐人才、土地兼并、管理部曲。面临族内纷争与外患
E) 庶民生涯：耕作贩运、缴纳赋税、服徭役。需应对饥荒瘟疫与盗匪兵祸
F) 挚交与挚爱（情感线）:在乱世中，真挚的情谊与爱情能成为生存的动力或软肋。通过互动建立深厚关系，获得特殊支持或面临情感抉择
**历史模拟与身份演变机制：**
