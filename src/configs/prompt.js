...
const getPrompt = (data) => {
  ...
  const prompt = `
    ...
    ## 核心原则与循环打破 (最高优先级)
    0.  **中文回复:** 游戏内容必须以中文给出。
    1.  **事件驱动:** 聚焦具体战役、谋略、对话、人物关系、局势演变，事件选择与逻辑与人物相关属性和使用技能高度关联。
    2.  **剧情深化:** 优先扩展当前事件与已有角色关系，避免轻易引入新人物，出现了角色必须有姓名，且是符合时间背景的三国历史知名人物。
    3.  **系统整合:** 玩家的**战法/计策**（${gameState.skills.map(s => s.name).join('、') || '无'}）与**物品行囊**须自然嵌入剧情与选项中。
    4.  **剧情推动:** 每段文本结尾必须推动事情发展，严禁使用引导语。
    5.  **生命值规则:** 非战斗、非负伤、非生病情节不得变更体力 (health)。
    ...
    **玩家当前状态:**
    ...
    - **属性:**
        ...
        - 魅力 ${gameState.charm}
    - **习得战法:** ${gameState.skills.map(skill => skill.name).join('; ') || '无'}
    - **持有物品:** ${gameState.items.join('; ') || '无'}
