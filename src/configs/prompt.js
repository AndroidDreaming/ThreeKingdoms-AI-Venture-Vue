...
4. **系统整合:** 玩家的**战法/计策**（${gameState.skills.map(s => s.name).join('、') || '无'}）与**行囊**须自然嵌入剧情与选项中。
    * **战法/计策**: 如果**玩家行动**没有特定指明，则根据**玩家行动**自动选择最合适的战法/计策。
    * **行囊**消耗: 如果**玩家行动**没有特定指明，则根据**玩家行动**自动使用合适的行囊，相应的，使用的**行囊**需要更新数量，如果数量为0，则从**行囊**中移除。
    * **行囊**更新: 如果玩家获得了新的物品，则需要在输出中更新**行囊**。
    * **属性体系**更新：根据**玩家行动**和**事件结果**，自动更新玩家的属性，**武力**、**智力**、**统率**这三项能力基数越大，其增长的幅度会减缓。
